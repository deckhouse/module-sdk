...
func NewValuesPatch() *ValuesPatch {
	return &ValuesPatch{
		Operations: make([]*utils.ValuesPatchOperation, 0),
	}
}
...
func (p *ValuesPatch) ToJSONPatch() (patch.Patch, error) {
	...
	patch, err := utils.DecodePatch(data)
	if err != nil {
		return nil, err
	}
	return patch, nil
...
func JSONPatchFromReader(r io.Reader) (patch.Patch, error) {
	operations := make([]patch.Operation, 0)

	dec := json.NewDecoder(r)
	for {
		var jsonStreamItem any
...
// TODO do we need a separate utils.ValuesPatchOperation type??
func ValuesPatchFromBytes(data []byte) (*ValuesPatch, error) {
	...
	var operations []*utils.ValuesPatchOperation
...
func CompactPatches(existedOperations []*utils.ValuesPatchOperation, newOperations []*utils.ValuesPatchOperation) ValuesPatch {
	...
	for _, op := range newOperations {
		...
		patchesTree[op.Path] = []*utils.ValuesPatchOperation{op}
	}
...
func FilterValuesPatch(valuesPatch ValuesPatch, rootPath string) ValuesPatch {
	resOps := []*utils.ValuesPatchOperation{}
...
func EnabledFromValuesPatch(valuesPatch ValuesPatch) ValuesPatch {
	resOps := make([]*utils.ValuesPatchOperation, 0)
