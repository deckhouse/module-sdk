...
var _ = Describe("metrics collector example", func() {
	...
	collector.AddMock.Set(func(name string, value float64, labels map[string]string, opts ...pkg.MetricCollectorOption) {
	...
	collector.SetMock.Set(func(name string, value float64, labels map[string]string, opts ...pkg.MetricCollectorOption) {
		Expect(name).Should(Equal("stub-set-metric"))
		Expect(value).Should(Equal(float64(1)))
		Expect(labels).Should(Equal(map[string]string{"node_found": "node_name"}))
	})

	collector.IncMock.Set(func(name string, labels map[string]string, opts ...pkg.MetricCollectorOption) {
